...
void sendDataPacket(const Data &data) {
  Serial.write(header, 2); // Send the header first
  const byte *byteData = (const byte*)&data;
  Serial.write(byteData, sizeof(Data)); // Then send the data
}
